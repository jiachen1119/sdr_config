...
myQchart::myQchart(QPointer<QCustomPlot> _parentPlot, PlotValueTracerType _type, QCPGraph *_graph):
        QObject(_parentPlot),
...
//设置游标点的样式
void myQchart::setTracerStyle(QCPItemTracer::TracerStyle e)
{
    if(m_tracer)
        m_tracer->setStyle(e);
}
//设置线元素的偏移长度
void myQchart::setArrowLineLength(int len)
{
    m_nLineLength = len;
    m_arrowLine->start->setCoords(m_nLineLength, 0);//偏移量
}
//设置线元素头样式
void myQchart::setArrowHead(QCPLineEnding::EndingStyle e)
{
    if(m_arrowLine)
        m_arrowLine->setHead(e);
}
//设置线元素的画笔
void myQchart::setArrowPen(const QPen &pen)
{
    if(m_arrowLine)
        m_arrowLine->setPen(pen);
}
//设置游标点的画笔
void myQchart::setTracerPen(const QPen &pen)
{
    if(m_tracer)
        m_tracer->setPen(pen);
}
//设置文本元素的画笔
void myQchart::setLabelPen(const QPen &pen)
{
    if(m_labelText)
        m_labelText->setPen(pen);
}
//设置游标点的画刷
void myQchart::setTracerBrush(const QBrush &brush)
{
    if(m_tracer)
        m_tracer->setBrush(brush);
}
//设置文本元素画刷
void myQchart::setLabelBrush(const QBrush &brush)
{
    if(m_labelText)
        m_labelText->setBrush(brush);
}
//设置文本元素文本
void myQchart::setText(const QString &text)
{
    if(m_labelText)
        m_labelText->setText(text);
}
//设置文本元素中文本的颜色
void myQchart::setTextColor(QColor &c)
{
    if(m_labelText)
        m_labelText->setColor(c);
}
//设置文本元素边界
void myQchart::setTextMargin(QMargins mar)
{
    if(m_labelText)
        m_labelText->setPadding(mar);
}
//设置游标的显示/隐藏
void myQchart::setVisible(bool visible)
{
    if(m_tracer)
...
//更新游标位置
void myQchart::updateTracerPosition(double x, double y)
{
    switch(m_type)
    {
        case XAxisTracer:
...
void myQchart::updatePositionX(double xValue)
{
    setVisible(true);
...
void myQchart::updatePositionY(double value)
{
    setVisible(true);
...
void myQchart::updatePositionData(double xValue, double yValue)
{
    setVisible(true);
